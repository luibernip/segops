...
def comparar(bit, pla):
    ...
    # 4. Completadas en Tasks pero el evento sigue OPEN en Bitácora
    abiertos = set(bit.loc[bit["Abierto"], "Evento"])
    r["task_comp_bit_open"] = pla[pla["Completada"] & pla["Evento"].isin(abiertos)]
...
def generar_html(bit, pla, r, salida, extras=None):
    ...
    conteos = {
        ...
        "Task completa / Bitácora OPEN": len(r["task_comp_bit_open"]),
    ...
    coa_con_inv = df_coa["Con investigación"].tolist()
    coa_sin_inv = df_coa["Sin investigación"].tolist()
    ...
    datos_js = json.dumps({
        ...
        "coa": {"labels": coa_labels, "open": coa_open, "prog": coa_prog,
                "con_inv": coa_con_inv, "sin_inv": coa_sin_inv},
        "dias_prom": dias_prom,
        "riesgos": riesgos,
        "por_coa": por_coa,
        "colores": COLORES,
  ...
  <div class="tabs-barra">
    ...
    secciones = "".join([
        ...
        seccion(4, "Tasks completadas con evento aún OPEN en Bitácora",
                "La tarea se cerró en Planner pero la ocurrencia sigue abierta",
        ...
        seccion(6, "Eventos OPEN en Bitácora que sí tienen Task",
                "Ocurrencias abiertas con tarea activa (no completada) en Planner",
                r["open_con_task"], cols_b),
    ])
...
<main>
  ...
  <div class="graficos">
    ...
    <div class="grafico"><h3>Investigación por COA (con / sin)</h3>
    ...
    <div class="grafico ancho"><h3>Resumen de hallazgos por categoría</h3>
...
const chCoaInv = new Chart(document.getElementById("gCoaInv"), {{
  ...
  data: {{ labels: D.coa.labels,
    datasets: [
      {{ label: "Con investigación", data: D.coa.con_inv, backgroundColor: C.azul_claro, borderRadius: 4 }},
      {{ label: "Sin investigación", data: D.coa.sin_inv, backgroundColor: C.rojo, borderRadius: 4 }}
    ] }},
...
const colorRiesgo = {{ "Intolerable": C.rojo_oscuro, "Unacceptable": C.rojo,
  "Tolerable": C.dorado, "Acceptable": "#5a8f5a",
  "Sin riesgo asignado": "#c9c9c9" }};
...
const chDias = new Chart(document.getElementById("gDias"), {{
  ...
  data: {{ labels: D.dias_prom.labels,
    datasets: [{{ label: "Días promedio", data: D.dias_prom.valores,
      backgroundColor: C.azul_claro, borderRadius: 6 }}] }},
...
const chBarras = new Chart(document.getElementById("gBarras"), {{
  type: "bar",
  data: {{ labels: Object.keys(D.conteos),
    datasets: [{{ data: Object.values(D.conteos),
...
function aplicarCoaGraficas(coa) {{
  ...
  chBarras.data.datasets[0].data = Object.values(d.conteos);
  ...
  chDias.data.datasets[0].backgroundColor = cols(C.azul_claro);
